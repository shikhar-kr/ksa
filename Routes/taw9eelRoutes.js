...
var routes = function (s) {
    ...
    var scrape = function(html){
        ...
        $('.products-grid .item').each(function (i, el) {
            try {
                item = {};
                item['site'] = s.site;
                item['href'] = $(this).find('.product-image').children('a').attr('href').split(/[?#]/)[0];
                item['image_tn'] = $(this).find('.product-image').find('img').data('echo');
                item['product'] = $(this).find('.product-image').children('a').attr('title');
                item['price'] = Number($(this).find('.price').html().split(/D/)[1].trim());
                item['stock'] = 1;
                prds.push(item);
            } catch (e) {
    ...
    productRouter.route('/Products')
        ...
        .get(function(req, res) {
            ...
            var options = {
                url: s.burl + '/en/catalogsearch/result/?q='+req.query.q,
                headers: {
                    'User-Agent': 'Request'
                }
            };
